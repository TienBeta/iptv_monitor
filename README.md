...
| Đường dẫn | Nội dung |
|---|---|
| `checker/` | Node.js, không thư viện ngoài: tải API (`source.js`), thông tin kênh cho sheet Streams (`details.js`), kiểm tra stream mức 1–4b (`check.js`, `http.js`), trạng thái (`status.js`), cầu nối Sheet (`bridge.js`), luồng chính (`index.js`) |
